...
void AerialMapDisplay::transformAerialMap()
{
  ...
  //   frame is used by OSM and Google Maps, see https://developers.google.com/maps/documentation/javascript/coordinates
  ...
  // unused variables
  Ogre::Vector3 translationUnused;
  Ogre::Quaternion rotationUnused;

  // rotation from map frame into the fixed frame
  Ogre::Quaternion rotationFfToMap;
  if (!context_->getFrameManager()->getTransform(frameMap, ref_fix_.header.stamp, translationUnused, rotationFfToMap))
  {
    // display error
    std::string error;
  ...
  // translation from nav sat fix frame to the fixed frame
  Ogre::Vector3 translationFfToNavSatFix;
  if (!context_->getFrameManager()->getTransform(frameNavSatFix, ref_fix_.header.stamp, translationFfToNavSatFix,
                                                 rotationUnused))
  {
    // display error
    std::string error;
  ...
  if (translationFfToNavSatFix.isNaN())
  {
